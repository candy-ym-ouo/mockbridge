...
func (h *MockHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	processCtx := context.WithoutCancel(r.Context())
	response := h.mocks.Process(processCtx, service.MockRequest{RequestID: middleware.RequestIDFrom(r.Context()), Method: r.Method, Path: r.URL.Path, Query: r.URL.Query(), RawQuery: r.URL.RawQuery, Headers: r.Header.Clone(), Body: body, ClientIP: clientIP(r)})
	...
	if response.Recordable {
		h.records.Submit(response.Record)
	}
}
